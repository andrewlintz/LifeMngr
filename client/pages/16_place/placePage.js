...
Template.placePage.helpers({
    placesbeenEach() {
        return DevTest.find( { placesbeenitemName: {$exists: true}}, {sort: {createdAt: -1}}); },
    placeswantEach() {
        return DevTest.find( { placeswantitemName: {$exists: true}}, {sort: {createdAt: -1}}); },
    placesplannerEach() {
        return DevTest.find( { placesplanneritemName: {$exists: true}}, {sort: {createdAt: -1}}); },
    improveSpacepageEach() {
        return DevTest.find( { improveSpacepageitemName: {$exists: true}}, {sort: {createdAt: -1}}); },
});
